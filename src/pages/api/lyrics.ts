...
export const GET: APIRoute = async ({ request }) => {
  ...
  try {
    const apiRes = await fetch(`https://api.lyrics.ovh/v1/${encodeURIComponent(artist)}/${encodeURIComponent(title)}`);
    const data = await apiRes.json();
    if (!apiRes.ok || !data.lyrics) {
      return new Response(JSON.stringify({ error: "Lyrics not found for this song." }), {
        status: 404,
        headers: { "Content-Type": "application/json" },
      });
    }
